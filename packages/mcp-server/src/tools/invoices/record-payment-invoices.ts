...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      mode: {
        type: 'string',
        description: 'ID du mode de paiement (PaymentMethod)',
      },
      jq_filter: {
        type: 'string',
        title: 'jq Filter',
...
export const handler = async (client: Wuro, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.invoices.recordPayment(uid, body)));
  } catch (error) {
