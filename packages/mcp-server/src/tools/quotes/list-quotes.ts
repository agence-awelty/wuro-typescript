...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      limit: {
        type: 'integer',
        description: 'Nombre maximum de devis à retourner',
      },
      maxDate: {
        type: 'string',
      ...
      skip: {
        type: 'integer',
        description: 'Nombre de devis à ignorer (pagination)',
      },
      sort: {
        type: 'string',
        description: 'Champ de tri et direction (ex. "date:-1")',
      },
      state: {
        type: 'string',
        description: 'Filtre par état du devis',
        enum: ['draft', 'pending', 'waiting', 'accepted', 'refused', 'invoiced', 'canceled', 'inactive'],
      },
      type: {
        type: 'string',
...
export const handler = async (client: Wuro, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.quotes.list(body)));
  } catch (error) {
