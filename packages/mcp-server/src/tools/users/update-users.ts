...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      avatar: {
        type: 'string',
        description: "URL ou fichier base64 de l'avatar",
      },
      birthdate: {
        type: 'string',
      ...
      title: {
        type: 'string',
        description: 'Civilité (MR, MME, etc.)',
      },
      jq_filter: {
        type: 'string',
        title: 'jq Filter',
...
export const handler = async (client: Wuro, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.users.update(uid, body)));
  } catch (error) {
