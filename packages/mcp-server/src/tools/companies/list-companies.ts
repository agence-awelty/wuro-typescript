...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      search: {
        type: 'string',
        description: "Recherche par nom d'entreprise (insensible à la casse)",
      },
      skip: {
        type: 'integer',
        description: "Nombre d'entreprises à ignorer (pagination)",
      },
      sort: {
        type: 'string',
      ...
      url: {
        type: 'string',
        description: 'Filtre par URL unique',
      },
      jq_filter: {
        type: 'string',
        title: 'jq Filter',
...
export const handler = async (client: Wuro, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.companies.list(body)));
  } catch (error) {
