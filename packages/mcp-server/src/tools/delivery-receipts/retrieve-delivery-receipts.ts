...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      populate: {
        type: 'string',
        description: 'Relations à inclure (ex. "client", "documentModel")',
      },
      jq_filter: {
        type: 'string',
        title: 'jq Filter',
...
export const handler = async (client: Wuro, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.deliveryReceipts.retrieve(uid, body)),
    );
  } catch (error) {
