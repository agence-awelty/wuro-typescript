...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      search: {
        type: 'string',
        description: 'Recherche textuelle dans nom, email, téléphone, code client',
      },
      skip: {
        type: 'integer',
        description: 'Nombre de clients à ignorer (pagination)',
      },
      sort: {
        type: 'string',
        description: 'Champ et direction de tri (ex. "name:1" pour tri alphabétique)',
      },
      state: {
        type: 'string',
        description: 'Filtrer par état (active = visible, inactive = archivé)',
...
export const handler = async (client: Wuro, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.clients.list(body)));
  } catch (error) {
